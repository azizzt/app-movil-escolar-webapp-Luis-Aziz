...
export class RegistroMaestrosComponent implements OnInit {
  ...
  ngOnInit(): void {
    ...
    // ---> 1. LÓGICA DE RELLENADO PARA EDICIÓN
    if(this.datos_user.id){
      this.editar = true;
      this.maestro = this.datos_user;
      
      // Corrección importante para las materias:
      // A veces el backend devuelve strings en lugar de arrays, aquí nos aseguramos
      if(typeof this.maestro.materias_json === 'string'){
          try{
              ...
              // Si falla o es un string plano, lo dejamos o inicializamos vacío
              // Si tu backend ya lo devuelve como lista, esto no es necesario, pero previene errores.
          }
      }
      // Si llega nulo, lo inicializamos como array vacío para que funcionen los checkbox
      if(!this.maestro.materias_json){
          this.maestro.materias_json = [];
      }

      console.log("Datos maestro a editar: ", this.maestro);
    }
  }
    ...
    if(this.maestro.password == this.maestro.confirmar_password){
      this.maestrosService.registrarMaestro(this.maestro).subscribe(
        (response) => {
          ...
          this.router.navigate(["home"]); // Simplificado
        },
        (error) => {
  ...
  // ---> 2. FUNCIÓN ACTUALIZAR
  public actualizar(){
    // Validación (segundo parámetro true = editar)
    this.errors = this.maestrosService.validarMaestro(this.maestro, true);
    if(Object.keys(this.errors).length > 0){
      return false;
    }

    console.log("Maestro a actualizar: ", this.maestro);
  ...
  public checkboxChange(event:any){
    // Aseguramos que materias_json sea un array antes de hacer push
    if(!this.maestro.materias_json){
        this.maestro.materias_json = [];
    }
