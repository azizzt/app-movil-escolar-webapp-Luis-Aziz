  ...
  public obtenerUserByID() {
    ...
    if(this.rol === "administrador"){
      ...
      this.maestrosService.getMaestroByID(this.idUser).subscribe(
        (response) => {
          console.log("Datos maestro recibidos:", response);
          this.user = response;
          // Mapeamos los datos del usuario base
          this.user.first_name = response.user.first_name;
          this.user.last_name = response.user.last_name;
          this.user.email = response.user.email;
          this.user.tipo_usuario = this.rol;
          // Esto activa el componente hijo <app-registro-maestros>
          this.isMaestro = true; 
        }, (error) => {
      ...
      this.alumnosService.getAlumnoByID(this.idUser).subscribe(
        (response) => {
          this.user = response;
          // Mapeamos los datos del usuario base para que sean accesibles fácilmente
          this.user.first_name = response.user.first_name;
          this.user.last_name = response.user.last_name;
          this.user.email = response.user.email;
          this.user.tipo_usuario = this.rol;
          // Esto activará el *ngIf en el HTML para mostrar el componente hijo con los datos cargados
          this.isAlumno = true; 
        }, (error) => {
