...
export class MaestrosService {
  ...
  public obtenerListaMaestros(page: number, pageSize: number, sort: string, search: string): Observable<any> {
    const token = this.facadeService.getSessionToken();
    let headers: HttpHeaders;
    ...
    // Configurar parámetros de consulta para el backend
    ...
    return this.http.get<any>(`${environment.url_api}/lista-maestros/`, { headers, params });
  }

  // ---> NUEVO: Obtener un solo maestro por ID (GET con query param)
  public getMaestroByID(idUser: number): Observable<any> {
    ...
    return this.http.get<any>(`${environment.url_api}/maestros/?id=${idUser}`, { headers });
  }

  // ---> NUEVO: Editar Maestro (PUT)
